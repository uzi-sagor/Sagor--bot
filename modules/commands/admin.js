    ...
    switch (args[0]) {
        ...
        case "-a": {
            ...
            for (const idAdmin of listAdmin) {
                if (parseInt(idAdmin)) {
                    const name = (await api.getUserInfo(idAdmin)).name
                    msg.push(`- ${name}\nUID: ${idAdmin}`);
                }
            }
        ...
        case "add": {
            if (event.senderID != global.config.ADMINBOT) return api.sendMessage(`Fuck!`, event.threadID, event.messageID)
        ...
        case "rm":
        case "delete": {
            if (event.senderID != global.config.ADMINBOT) return api.sendMessage(`Fuck off!`, event.threadID, event.messageID)
