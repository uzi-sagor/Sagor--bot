...
module.exports.message = async (api, event) => {
    return {
      ...
      react: async (emoji, messageID = event.messageID, callback) => {
        try {
      ...
      edit: async (newContent = "🐸🐸", messageID) => {
        try {
          messageID =
            messageID || event?.messageReply.messageID || event.messageID;
          return await api.editMessage(newContent, messageID);
        } catch (err) {
          throw err;
        }
      },
